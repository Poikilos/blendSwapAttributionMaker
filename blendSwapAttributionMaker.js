...
*/
  ...
  var datePartsMin = 3; // 3 for blendswap.com, 4 for opengameart
  var dateLabel = "Published: ";
  // blendswap.com:
  /*
  ```
  ...
  */
  ...
  function getElementsByTagWhereChildHasClass(tagName, className) {
    ...
    for (var i=0, max=all.length; i < max; i++) {
      ...
      if (typeof (childClassName) != "string") {
        ...
        if (verbose) {
          if (tagName != "*") {
            console.log("  - typeof el.className is " + typeof (child.className) + " for tagName " + child.tagName)
            // Possible outcomes:
            // className type can be svg or path as opposed to string (WARNING:
            // `<i` becomes `<svg` in FontAwesome!!)
  ...
  function getMarkdown(info) {
    ...
    // TODO: Move the getAnchorsWhereHrefContains call to the info gathering function.
  ...
  function populateCorrespondingLicenseFields(license) {
    ...
    if (license.href) {
      ...
      if (!licenseShortStr) {
        console.warn("The URL \""+license.href+"\" is not recognized (No key in "+myName+"'s urlSmallNames is a partial of the URL), so the long license name could not be generated.");
      }
      else {
        ...
        if (!license.textContent) {
          ...
          if (bigNames.hasOwnProperty(licenseShortStr)) {
            ...
            console.warn("The short license name \""+licenseShortStr+"\" is not recognized (It is not a key in Thing Remix Attribution Maker's bigNames), so the long license name could not be generated.");
          }
        }
      }
    }
  ...
  function getInfo() {
    ...
    */
    ...
    if (programVersionTag && programVersionFlag) {
      ...
      if (programEls.length > 1) {
        console.warn("There were more than one "+programVersionTerm+" elements ("+programVersionTag+" containing \""+programVersionFlag+"\"");
      }
    }
    if (mediumTag && mediumFlag) {
      ...
      if (mediumEls.length > 1) {
        console.warn("There were more than one "+mediumTerm+" elements ("+mediumTag+" containing \""+mediumFlag+"\"");
      }
    }
    if (descriptionTag && descriptionClassName) {
      ...
      if (descriptionEls.length > 1) {
        console.warn("There were more than one description elements (tagName "+descriptionTag+" where className starts with \""+descriptionClassName+"\"");
      }
    }
    ...
    */
    if (submittedDateStr !== null) {
      var createdParts = submittedDateStr.split(" ");
      if (createdParts.length >= datePartsMin) {
        ...
        console.warn("There were only " + createdParts.length + " parts in the date, so it was assumed to be unparseable and skipped.");
      }
    }
    else {
      console.warn("No date string was found.");
    }
